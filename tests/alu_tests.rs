...
fn test_alu_add_16_overflow() {
    ...
    let res = cpu.alu_add(2, (0x7FFF) as u32, (1) as u32, false);
...
fn test_alu_sub_8() {
    ...
    // 10 - 5 = 5
    let res = cpu.alu_sub(1, (10) as u32, (5) as u32, false);
    assert_eq!(res, 5);
    assert_eq!(cpu.get_cpu_flag(CpuFlags::ZF), false);

    // 5 - 10 = -5 (0xFB)
    let res = cpu.alu_sub(1, (5) as u32, (10) as u32, false);
    assert_eq!(res, 0xFB);
...
fn test_parity_flag() {
    ...
    let res = cpu.alu_add(2, (0x0100) as u32, (0x0003) as u32, false);
...
fn test_alu_sbb_ripple_borrow() {
    ...
    let low = cpu.alu_sub(1, (0x00) as u32, (0x01) as u32, false);
    assert_eq!(low, 0xFF);
    assert_eq!(cpu.get_cpu_flag(CpuFlags::CF), true); // Borrow out
    assert_eq!(cpu.get_cpu_flag(CpuFlags::ZF), false);

    // Step 2: High byte 0x01 - 0 (with borrow)
    let high = cpu.alu_sub(1, (0x01) as u32, (0x00) as u32, cpu.get_cpu_flag(CpuFlags::CF));
    assert_eq!(high, 0x00);
...
fn test_alu_sbb_16_complex() {
    ...
    let res = cpu.alu_sub(2, (0x0000) as u32, (0x0000) as u32, cpu.get_cpu_flag(CpuFlags::CF));
    ...
    // Result: 0, CF=0, ZF=1
    cpu.set_cpu_flag(CpuFlags::CF, true);
    let res = cpu.alu_sub(2, (0x0001) as u32, (0x0000) as u32, cpu.get_cpu_flag(CpuFlags::CF));
...
fn test_alu_adc_8_overflow() {
    ...
    // 254 + 1 + (CF=1) = 256 -> 0
    cpu.set_cpu_flag(CpuFlags::CF, true);
    let res = cpu.alu_add(1, (254) as u32, (1) as u32, cpu.get_cpu_flag(CpuFlags::CF));
    assert_eq!(res, 0);
    assert_eq!(cpu.get_cpu_flag(CpuFlags::ZF), true);
    assert_eq!(cpu.get_cpu_flag(CpuFlags::CF), true);
}
