...
fn test_vram_mapping() {
    ...
    assert_eq!(bus.vram_text[0], 0x41);
    ...
    assert_eq!(bus.vram_graphics[0], 0x00);

    // 2. Test Graphics Mode VRAM (0xA0000)
    let graph_addr = ADDR_VGA_GRAPHICS; // 0xA0000
    bus.write_8(graph_addr, 0xFF);
    
    assert_eq!(bus.vram_graphics[0], 0xFF);
...
fn test_pit_channel_0_reset_bug() {
    ...
    assert!(bus.pit0_write_msb, "PIT0 toggle should be TRUE (expecting MSB)");
    ...
    assert_eq!(bus.pit0_write_msb, false, "PIT Channel 0 latch failed to reset after Command 0x36!");
